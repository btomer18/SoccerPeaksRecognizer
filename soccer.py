...
def filter_important_words(words: List[Word], exiting_words: List[str]) -> List[float]:
    return [word.start for word in words if word.word in exiting_words]
...
def main():
    ...
    exiting_words = Path("exciting_words.txt").read_text().splitlines()
    text = audio_to_text(filename)
    sound_peaks = audio_peaks(filename)
    text_peaks = filter_important_words(text, exiting_words)
